...
import {SiJavascript, SiGithub, SiTailwindcss, SiReact} from 'react-icons/si'
...
function TechStack() {
  ...
  return (
    ...
    className={`w-full  ${darkMode ? 'bg-cyan-900' : 'bg-sky-100'}`}>
        <div className={`max-w-[1040px] m-auto md:pl-20 p-4 py-16 ${darkMode ? 'bg-cyan-900' : 'bg-sky-100'}`}>
            <h1 className={`text-4xl md:text-5xl lg:text-6xl pb-6 font-bold text-center ${darkMode 
            ? 'text-sky-300' 
            : 'text-cyan-800'}`}>
            Skills
            </h1>
            <div className='grid grid-cols-2 gap-12 lg:grid-cols-3 items-center text-center justify-between m-3 p-3 pl-12 my-6'>
